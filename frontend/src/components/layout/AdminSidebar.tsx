...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ collapsed, onToggle }) => {
  ...
  return (
    <aside
      className={cn(
        'admin-sidebar fixed inset-y-0 left-0 h-[100dvh] bg-sidebar border-r border-sidebar-border flex flex-col overflow-hidden z-40 w-[17rem] sm:w-64 transform transition-all duration-300',
    ...
    >
      ...
      <div
          className={cn(
            ...
            collapsed ? 'justify-center p-3 md:p-4' : 'p-3 md:p-4'
          )}
      >
        <Link to="/?view=landing" className={cn('flex items-center gap-3 group', collapsed && 'justify-center')}>
          <img
            ...
            width={40}
            height={40}
            className="w-9 h-9 md:w-10 md:h-10 rounded-lg object-contain shadow-sm"
          />
          {!collapsed && (
            <div>
        ...
        {visibleNavItems.map((item) => {
          ...
          const link = (
            <Link
              ...
              className={cn(
                'group relative flex items-center gap-2.5 md:gap-3 px-2.5 md:px-3 py-2 md:py-2.5 rounded-lg text-[13px] md:text-sm font-medium transition-all duration-200',
                isActive(item.href)
        ...
        <div className={cn('flex items-center gap-3 mb-3', collapsed && 'justify-center')}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-[13px] md:text-sm font-medium text-sidebar-foreground truncate">
                {user?.name}
              </p>
              <p className="text-[11px] md:text-xs text-muted-foreground truncate">{user?.email}</p>
            </div>
          )}
        </div>
