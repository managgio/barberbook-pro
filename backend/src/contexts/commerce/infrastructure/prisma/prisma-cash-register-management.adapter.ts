...
import { CommerceCashMovement } from '../../domain/entities/cash-movement.entity';
import {
...
const mapCashMovement = (movement: CashMovementWithItems): CommerceCashMovement => ({
  id: movement.id,
...
@Injectable()
export class PrismaCashRegisterManagementAdapter
  implements CommerceCashRegisterManagementPort
{
  constructor(private readonly prisma: PrismaService) {}

  async listByDate(params: { localId: string; date: string }): Promise<CommerceCashMovement[]> {
    const from = startOfDay(params.date);
    const to = endOfDay(params.date);
    const movements = await this.prisma.cashMovement.findMany({
      where: { localId: params.localId, occurredAt: { gte: from, lte: to } },
  ...
  async create(data: CreateCashMovementInput): Promise<CommerceCashMovement> {
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      const resolvedProductItems = operation
        ? await this.resolveProductItems(tx, data.localId, normalizedProductItems)
        : [];
      ...
      const movement = await tx.cashMovement.create({
        data: {
          ...
          note: note || null,
          occurredAt,
          productOperationType: (operation ?? null) as CashMovementProductOperationType | null,
          productItems:
  ...
  async update(data: UpdateCashMovementInput): Promise<CommerceCashMovement> {
    const existing = await this.prisma.cashMovement.findFirst({
      where: { id: data.movementId, localId: data.localId },
    ...
    const updated = await this.prisma.cashMovement.update({
      ...
      data: {
        type: data.type as CashMovementType | undefined,
  ...
  async remove(params: { localId: string; movementId: string }): Promise<{ success: true }> {
    const existing = await this.prisma.cashMovement.findFirst({
      where: { id: params.movementId, localId: params.localId },
      include: {
        productItems: true,
      },
    });
    ...
    await this.prisma.$transaction(async (tx) => {
      if (existing.productOperationType && existing.productItems.length > 0) {
        ...
        const currentProducts = existingIds.length
          ? await tx.product.findMany({
              where: { id: { in: existingIds }, localId: params.localId },
      ...
      await tx.cashMovement.delete({ where: { id: params.movementId } });
    });
    return { success: true };
  }

  private getOperationMovementType(
    operation: CashMovementProductOperationType,
  ): CashMovementType {
    return operation === 'sale' ? 'in' : 'out';
  }

  private normalizeProductItems(items?: CreateCashMovementInput['productItems']) {
