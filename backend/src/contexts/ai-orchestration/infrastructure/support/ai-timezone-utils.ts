export const AI_MEMORY_DEFAULT_TIME_ZONE = 'Europe/Madrid';
...
export const getDateStringInTimeZone = (date: Date, timeZone = AI_MEMORY_DEFAULT_TIME_ZONE) => {
  const parts = buildParts(date, timeZone);
  const pad = (value: number) => value.toString().padStart(2, '0');
  return `${parts.year}-${pad(parts.month)}-${pad(parts.day)}`;
};

export const getDayBoundsInTimeZone = (dateString: string, timeZone = AI_MEMORY_DEFAULT_TIME_ZONE) => {
