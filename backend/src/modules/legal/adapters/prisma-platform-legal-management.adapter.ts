...
import {
  ...
  PlatformLegalContentResponse,
  PlatformLegalCustomSections,
  PlatformLegalManagementPort,
  PlatformLegalPageType,
  PlatformLegalSection,
  PlatformLegalSettingsResolved,
  PlatformLegalSubProcessor,
...
const sanitizeSubProcessors = (value: unknown): PlatformLegalSubProcessor[] => {
  if (!Array.isArray(value)) return [];
  const sanitized = value.map((entry): PlatformLegalSubProcessor | null => {
  ...
  return sanitized.filter((entry): entry is PlatformLegalSubProcessor => Boolean(entry));
};

const sanitizeCustomSections = (value: unknown): PlatformLegalCustomSections => {
  if (!value || typeof value !== 'object') return {};
  const raw = value as Record<string, unknown>;
  const sanitizeSections = (
    sections: unknown,
  ): PlatformLegalCustomSections[keyof PlatformLegalCustomSections] => {
    if (!Array.isArray(sections)) return undefined;
    const sanitized = sections.map((section): PlatformLegalSection | null => {
    ...
    return sanitized.filter((section): section is PlatformLegalSection => Boolean(section));
  };
...
@Injectable()
export class PrismaPlatformLegalManagementAdapter implements PlatformLegalManagementPort {
  constructor(
  ...
  async getSettings(brandId?: string, localId?: string | null): Promise<PlatformLegalSettingsResolved> {
    const resolvedBrandId = brandId ?? this.getBrandId();
    const resolvedLocalId = localId === undefined ? this.getLocalId() : localId;
    const settings = await this.ensureSettings(resolvedBrandId, resolvedLocalId || undefined);
    return this.resolveSettings(settings, resolvedBrandId, resolvedLocalId || undefined);
  }
  ...
  async updateSettings(
    ...
    const resolvedBrandId = brandId ?? this.getBrandId();
    const resolvedLocalId = localId === undefined ? this.getLocalId() : localId;
    await this.ensureSettings(resolvedBrandId, resolvedLocalId || undefined);
    ...
    const payload: Prisma.BrandLegalSettingsUpdateInput = {
      ...
      optionalCustomSections: data.optionalCustomSections
        ? sanitizeCustomSections(data.optionalCustomSections)
        : undefined,
    ...
    return this.resolveSettings(updated, resolvedBrandId, resolvedLocalId || undefined);
  }
  ...
  async getPolicyContent(
    ...
    const settings = await this.getSettings(brandId ?? this.getBrandId(), localId ?? this.getLocalId());
    return buildLegalContent(type, settings, settings.optionalCustomSections) as PlatformLegalContentResponse;
  }
  ...
  async recordPrivacyConsent(params: {
    ...
    const locationId = params.locationId ?? this.getLocalId();
  ...
  async hasUserPrivacyConsent(userId: string, policyVersion?: number, brandId?: string): Promise<boolean> {
    const resolvedBrandId = brandId ?? this.getBrandId();
    const version = policyVersion ?? (await this.getSettings(resolvedBrandId)).privacyPolicyVersion;
  ...
  async getDpaContent(brandId: string): Promise<PlatformLegalContentResponse> {
    const settings = await this.getSettings(brandId, null);
    return buildLegalContent('dpa', settings, settings.optionalCustomSections) as PlatformLegalContentResponse;
  }
...
export const PrismaPlatformLegalManagementAdapterProvider = {
  provide: PLATFORM_LEGAL_MANAGEMENT_PORT,
  useExisting: PrismaPlatformLegalManagementAdapter,
};
