...
type ApiMetricRecord = PlatformApiMetricRecord;
...
export const resolveObservabilityEnvironmentLabel = (env: NodeJS.ProcessEnv = process.env) => {
  const raw =
    env.OBSERVABILITY_RUNTIME_ENV ||
    env.APP_ENV ||
    env.VERCEL_ENV ||
  ...
  if (
    ...
    normalized.startsWith('prod_')
  ) {
    return 'prod';
  }
  if (normalized.includes('local') || normalized.includes('dev') || normalized.includes('test')) {
    return 'local';
  }
  return normalized;
};
...
@Injectable()
export class InMemoryPrismaPlatformObservabilityAdapter
  implements PlatformObservabilityPort, OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(InMemoryPrismaPlatformObservabilityAdapter.name);
  ...
  private readonly runtimeEnvironment = resolveObservabilityEnvironmentLabel();
  ...
  private async maybeAlertApiDegradation(record: ApiMetricRecord) {
    ...
    void this.sendAlertEmail({
      subject: `[ALERTA][${this.runtimeEnvironment.toUpperCase()}][API] Degradacion ${record.method} ${record.route}`,
      lines,
    });
  }
  ...
  recordWebVital(
    ...
    if (payload.rating === PlatformWebVitalRating.POOR) {
      this.logger.warn(
  ...
  async getWebVitalsSummary(windowMinutes?: number) {
    ...
    recent.forEach((entry) => {
      if (entry.rating !== PlatformWebVitalRating.POOR) return;
