...
import {
  CommerceAssignUserSubscriptionInput,
  CommerceCreateSubscriptionPlanInput,
  CommerceMarkSubscriptionPaidInput,
  CommerceSubscribePlanInput,
  CommerceSubscriptionCheckoutMode,
  CommerceSubscriptionManagementPort,
  CommerceUpdateSubscriptionPlanInput,
...
@Injectable()
export class PrismaStripeSubscriptionManagementAdapter implements CommerceSubscriptionManagementPort {
  constructor(
  ...
  private async getStripeAccountIdForSubscriptionCheckout() {
    ...
    const accountStatus = await this.stripeGateway.getAccountStatus({ accountId });
  ...
  private async createStripeCheckoutForSubscription(params: {
    ...
    const session = await this.stripeGateway.createCheckoutSession({
      ...
      unitAmount,
      serviceName: `Suscripción · ${params.planName}`,
      customerEmail: params.userEmail,
      clientReferenceId: params.subscriptionId,
      metadata: {
      ...
      successUrl: `${params.baseUrl}/app/subscriptions?subscriptionPayment=success&session_id={CHECKOUT_SESSION_ID}`,
      cancelUrl: `${params.baseUrl}/app/subscriptions?subscriptionPayment=cancel&session_id={CHECKOUT_SESSION_ID}`,
    });
  ...
  async createPlan(data: CommerceCreateSubscriptionPlanInput) {
    await this.assertEnabled();
    const localId = this.getLocalId();
    ...
    const created = await this.prisma.subscriptionPlan.create({
      data: {
        ...
        durationUnit: this.asDurationUnit(data.durationUnit, 'durationUnit'),
  ...
  async updatePlan(id: string, data: CommerceUpdateSubscriptionPlanInput) {
    await this.assertEnabled();
    const localId = this.getLocalId();
      ...
      data: {
        ...
        durationUnit:
          data.durationUnit === undefined
            ? undefined
            : this.asDurationUnit(data.durationUnit, 'durationUnit'),
  ...
  private async createUserSubscription(
    ...
    data: CommerceAssignUserSubscriptionInput | CommerceSubscribePlanInput,
    options?: {
    ...
    const paymentMode = options?.paymentMode ?? 'next_appointment';
    const paymentStatus =
      source === UserSubscriptionSource.client
        ...
        : PaymentStatus.paid;
    const paymentMethod =
      source === UserSubscriptionSource.client && paymentMode === 'stripe'
  ...
  async assignUserSubscription(userId: string, data: CommerceAssignUserSubscriptionInput) {
    await this.assertEnabled();
  ...
  async subscribeCurrentUser(userId: string, data: CommerceSubscribePlanInput, baseUrl: string) {
    await this.assertEnabled();
    const paymentMode = data.paymentMode ?? 'next_appointment';
  ...
  async markSubscriptionPaid(userId: string, subscriptionId: string, data: CommerceMarkSubscriptionPaidInput) {
    await this.assertEnabled();
    const localId = this.getLocalId();
    await this.ensureUserExists(userId);
    ...
    const updated = await this.prisma.userSubscription.update({
      ...
      data: {
        ...
        paymentMethod: this.asPaymentMethod(data.paymentMethod) ?? existing.paymentMethod,
  ...
  async settlePendingInPersonPaymentFromAppointment(params: {
    ...
    const updated = await this.prisma.userSubscription.update({
      ...
      data: {
        paymentStatus: PaymentStatus.paid,
        paymentMethod: this.asPaymentMethod(params.paymentMethod) ?? existing.paymentMethod,
        paymentAmount: existing.paymentAmount ?? existing.plan.price,
        paymentCurrency: existing.paymentCurrency ?? DEFAULT_CURRENCY,
        paymentPaidAt: paidAt,
      },
  ...
  private asDurationUnit(value: string, field: string): SubscriptionDurationUnit {
    if ((Object.values(SubscriptionDurationUnit) as string[]).includes(value)) {
      return value as SubscriptionDurationUnit;
    }
    throw new BadRequestException(`${field} no es una unidad de duración válida.`);
  }
}
