...
import { PlatformUsageMetrics, ProviderDailySeriesPoint } from '../../../contexts/platform/domain/entities/platform-usage-metrics.entity';
import {
...
import {
  ...
  parseDateString,
} from './usage-metrics-timezone.utils';
...
@Injectable()
export class PrismaPlatformUsageMetricsAdapter implements PlatformUsageMetricsPort {
  private readonly logger = new Logger(PrismaPlatformUsageMetricsAdapter.name);
  ...
  async recordOpenAiUsage(params: PlatformOpenAiUsageInput) {
  ...
  async recordTwilioUsage(params: PlatformTwilioUsageInput) {
    const brandId = params.brandId;
    const dateKey = getDateStringInTimeZone(new Date(), AI_TIME_ZONE);
    await this.upsertUsage({
      provider: 'twilio',
  ...
  async recordImageKitUsage(params: PlatformImageKitUsageInput) {
    const dateKey = getDateStringInTimeZone(new Date(), AI_TIME_ZONE);
    await this.upsertUsage({
      provider: 'imagekit',
