...
import {
  ...
  EngagementReferralConfigPayload,
  EngagementUpdateReferralConfigInput,
...
@Injectable()
export class PrismaTenantReferralConfigManagementAdapter implements EngagementReferralConfigManagementPort {
  constructor(
  ...
  private mapConfig(config: any | null, services?: Record<string, string>): EngagementReferralConfigPayload {
  ...
  async getConfig(): Promise<EngagementReferralConfigPayload> {
  ...
  async updateConfig(data: EngagementUpdateReferralConfigInput) {
    if (!(await this.isModuleEnabled())) {
      throw new BadRequestException('El programa de referidos no está habilitado en este local.');
    }
    const localId = this.getLocalId();
