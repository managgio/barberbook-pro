...
import {
  EngagementAttributeReferralInput,
  EngagementReferralAttributionManagementPort,
  EngagementReferralListParams,
...
@Injectable()
export class PrismaReferralAttributionManagementAdapter implements EngagementReferralAttributionManagementPort {
  constructor(
  ...
  async getReferrerSummary(userId: string) {
    ...
    const config = await this.configService.getConfig();
    const moduleEnabled = await this.configService.isModuleEnabled();
    const blockedBySubscription = await this.commerceSubscriptionPolicyPort.hasUsableActiveSubscription(
      userId,
      new Date(),
    );
  ...
  async attributeReferral(data: EngagementAttributeReferralInput) {
  ...
  async listReferrals(params: EngagementReferralListParams) {
    const localId = this.getLocalId();
