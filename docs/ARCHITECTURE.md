...
Modulos principales:
- **Tenancy**: resolucion de tenant, bootstrap (`/tenant/bootstrap`) y HTML de metadatos para crawlers (`/tenant/preview`).
...
- **Chunk i18n cacheable**: los catálogos de copy fijo se emiten como chunk `app-i18n`, separado del entry de aplicación. Así el crecimiento de traducciones no degrada ni invalida el núcleo de arranque en cada despliegue y conserva los presupuestos de rendimiento del entry.
...
- **Fecha por defecto en Buscar citas (sin storage)**: `AdminSearch` inicializa siempre el filtro de fecha en el día actual (`yyyy-MM-dd`) al entrar/cambiar de local y ya no persiste fecha en `localStorage` (evita recuperar filtros antiguos por sesión).
...
- **Color por profesional en calendario**: cada barbero puede tener `calendarColor` persistido (editable desde `AdminBarbers` con click en indicador de color), y `AdminCalendar` reutiliza ese color en los bloques de cita; si no existe color configurado, aplica fallback determinista por `barberId`.
- **Resiliencia de snapshots en citas**: listados de citas (calendar/search/clients y cliente) priorizan `barberNameSnapshot`/`serviceNameSnapshot` cuando el recurso ya no está en catálogo activo (ej. barbero archivado), evitando perder contexto histórico tras bajas.
- **Indicador de notas de cita adaptativo por dispositivo**: el icono de comentario en citas mantiene tooltip por hover/focus en desktop y usa popover por click en pantallas táctiles (tablet/móvil) para preservar accesibilidad y legibilidad sin depender de hover.
...
- **Contacto coherente entre configuración y landing**: en `AdminSettings` el prefijo telefónico queda fijo en `+34` (no editable) y solo se persiste `contact.phone` cuando existe número real; en `LandingPage` el teléfono del footer solo se muestra si hay número (no basta con prefijo), y la columna `Síguenos` se oculta completamente cuando no hay redes configuradas.
...
- **Semántica de error API para observabilidad**: `errorRate` en Top endpoints se calcula con errores de servidor (`statusCode >= 500`), evitando clasificar como degradación técnica los 4xx esperados de negocio/autorización.
- **Hardening de alertas Web Vitals**: el backend descarta outliers no razonables por métrica (picos anómalos), suprime alertas de user-agents in-app conocidos (Instagram/Facebook/TikTok/LINE) y excluye paths de plataforma (`/platform*`) para reducir ruido operativo en producción.
...
- **Informe PDF de observabilidad**: la cabecera de Platform Observability permite generar bajo demanda un PDF A4 horizontal para la ventana activa (`60 min | 24 h | 7 días`). Antes de exportar fuerza una lectura fresca de Web Vitals/API e incluye entorno runtime, rango exacto, resumen ejecutivo, percentiles/ratings, desglose Web Vitals por tenant/local/ruta con primeras/últimas muestras, Top endpoints por cliente con distribución HTTP y muestras más lentas. `jspdf` y `jspdf-autotable` se cargan mediante import dinámico y chunk `vendor-pdf`, sin penalizar el entry ni las rutas tenant.
- **Trazas críticas extensibles y orientadas a error**: Platform dispone de una superficie transversal de trazas críticas categorizadas. Actualmente el productor activo es `booking`: cada intento usa un `traceId` común y conserva selección de servicio, profesional, fecha/hora, render de confirmación y envío como breadcrumbs acotados en memoria del navegador. Si el flujo termina bien se descartan; si falla se persiste una única traza con el recorrido compacto, contexto funcional y detalle técnico. `AppErrorBoundary` adjunta las excepciones React con mensaje/stack. La identidad se resuelve en backend desde el token y cada error conserva tenant, local, usuario, ruta, user-agent y timestamp. El caso de uso backend rechaza la persistencia de hitos correctos como defensa ante clientes antiguos. La tabla usa paginación real de servidor (`page/pageSize`) y el PDF obtiene un lote operativo acotado; no se cargan todas las trazas de la ventana en el navegador.
- **Fiabilidad y retención de telemetría crítica**: `POST /api/observability/critical-traces` persiste cada evento directamente en MySQL y cualquier fallo de telemetría se aísla del flujo principal. `GET /api/platform/observability/critical-traces` consulta la ventana de forma paginada y `PATCH /api/platform/observability/critical-traces/preferences` persiste su inclusión en PDF. Las trazas críticas se eliminan automáticamente al superar 90 días (`OBSERVABILITY_CRITICAL_TRACE_RETENTION_DAYS`, configurable) bajo lock distribuido y limpieza periódica; no comparten necesariamente la retención de métricas agregadas.
...
CriticalTraceEvent | Trazas críticas extensibles | Hitos y errores por categoría (`booking` inicialmente), correlacionados por intento, tenant/local, usuario y contexto funcional
ObservabilityPreference | Preferencias operativas de observabilidad | Configuración persistida de inclusión de bloques especializados en el informe PDF
...
2) **Login y sincronizacion**
   ...
   - El requisito opcional `SiteSettings.profile.phoneRequired` actúa como guard global del tenant: cuando está activo, cualquier cliente autenticado sin teléfono (incluidos usuarios registrados previamente) ve un modal no descartable desde cualquier vista tenant y debe guardar el teléfono para continuar. Se excluyen usuarios con acceso admin/plataforma y, si el requisito está desactivado, el modal no se renderiza. En `ClientLayout` este guard tiene prioridad sobre el prompt de reseñas para evitar modales simultáneos.
...
5) **Notificaciones**
   ...
   - El render de fecha/hora en comunicaciones usa `APP_TIMEZONE` (no la timezone del servidor) para evitar desfases en correos y recordatorios.
...
16) **Sincronizacion de cache UI (React Query + eventos admin)**
   ...
   - UX móvil de `AuthPage`: formulario/tab switcher/CTA y bloques auxiliares usan densidad compacta en `<sm` (tipografía, alturas de inputs/botones y spacing) para reducir scroll y ruido visual sin alterar el layout desktop.
   - UX de `BookingWizard` (cliente/guest): al seleccionar un horario en el paso 2, el flujo avanza automáticamente al paso 3 (confirmación), eliminando el clic manual en `Siguiente` y reduciendo fricción en móvil.
...
- Observability persistence: `OBSERVABILITY_PERSIST_FLUSH_MS`, `OBSERVABILITY_PERSIST_BATCH_SIZE`, `OBSERVABILITY_PERSIST_BUFFER_LIMIT`, `OBSERVABILITY_PERSIST_RETENTION_DAYS`, `OBSERVABILITY_CRITICAL_TRACE_RETENTION_DAYS` (default 90), `OBSERVABILITY_SUMMARY_QUERY_CAP`.
...
- Contrato de staff:
  - `GET /api/barbers` devuelve únicamente barberos `isActive=true` y `isArchived=false`.
  - `GET /api/barbers/admin` (solo admin) devuelve barberos activos + inactivos (`isArchived=false`) para gestión interna.
...
- Observabilidad backend:
  ...
  - `POST /api/observability/critical-traces` registra hitos/errores categorizados y `GET /api/platform/observability/critical-traces?minutes=&page=&pageSize=` los consulta con paginación; la preferencia PDF se persiste vía `PATCH /api/platform/observability/critical-traces/preferences`.
...
- Indices operativos adicionales para volumen alto:
  ...
  - `CriticalTraceEvent(brandId, localId, occurredAt)`, `(traceId, occurredAt)`, `(userId, occurredAt)` y `(outcome, occurredAt)`
...
- E2E smoke runtime existe y se ejecuta con `npm run test:e2e:smoke`.
  ...
  - Hardening anti-flake en `runtime-authenticated-smoke`: `auth.appointments.create.valid` no depende de un único slot; prueba varios candidatos (límite acotado) para reducir falsos negativos por desincronización puntual de disponibilidad.
