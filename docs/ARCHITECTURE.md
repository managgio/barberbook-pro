...
- **Trazas críticas extensibles**: Platform dispone de una superficie transversal de trazas críticas categorizadas. Actualmente el productor activo es `booking`: cada intento usa un `traceId` común y registra selección de servicio, profesional, fecha/hora, render de confirmación, envío y resultado. `AppErrorBoundary` adjunta las excepciones React con mensaje/stack. La identidad se resuelve en backend desde el token y cada evento conserva tenant, local, usuario, contexto funcional, ruta, user-agent y timestamp. La tabla usa paginación real de servidor (`page/pageSize`) y el PDF obtiene un lote operativo acotado; no se cargan todas las trazas de la ventana en el navegador.
